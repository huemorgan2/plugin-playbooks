...
async def test_delegate_always_runs_as_building(env):
    # plans/020: luna 098 collapsed states to planning/building. The delegate
    # always runs as "building" — even when spawned from a planning chat —
    # because its whole job is building; containment is the explicit tools
    # allowlist plus the machine-checked publish gates.
    agent = FakeAgent(result="done")
    tools = _tools(FakeCtx(agent, state="planning"), env)
    _, run = tools["playbook_agent"]
    out = json.loads(await run(task="job", wait_seconds=10))
    assert out["status"] == "done"
    assert agent.calls[0]["conversation_state"] == "building"
